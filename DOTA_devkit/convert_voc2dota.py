    ...
    for label_path in xml_paths:
        ...
        with open(f"{folder_out_path}/{file_name}.txt", "w") as f:
            for box in boxes:
                ...
                if angle <= math.pi / 2:
                    f.write(
                        f"{str(p3x)} {str(p3y)} {str(p0x)} {str(p0y)} {str(p1x)} {str(p1y)} {str(p2x)} {str(p2y)} {name} 0\n"
                    )
                else:
                    f.write(
                        f"{str(p1x)} {str(p1y)} {str(p2x)} {str(p2y)} {str(p3x)} {str(p3y)} {str(p0x)} {str(p0y)} {name} 0\n"
                    )
...
def train_val_split(src_path, out_path, val_ratio=0.1):
    ...
    print(
        f"train dataset: {len(image_paths) - val_num} images\nvalid dataset: {val_num} images"
    )
...
def parse_args():
    ...
    parser.add_argument(
        "--function",
        help="1: convert_voc2dota, 2: split_data_have_label, 3: train_val_split",
        type=str,
        default=r"1",
    )
    parser.add_argument(
